...
$(document).ready(function() {
    ...
    if (userID == "") {
        $('#loginAndFavourites').css('display','none');
        var formLogin= $('#login'),
        formSignup=$('#signup'),
        formSwitch=$('.switcher'),
...
function new_fav_capture() {
    $('#add_favourite').submit(function() {
        ...
        var new_fav_data = {
            'name': document.getElementById('fav_name').value,
            'description': document.getElementById('fav_description').value,
...
function login() {
    FB.getLoginStatus(function(response) {
        if (response && response.status == 'connected') {
            ...
            // $('#main_content').append('<p>Please register.</p>');
        }
    });
}
        ...
        if (!response) {
            ...
            FB.login(function(response) {
                ...
                FB.api('/me?fields=email,name', function(response) {
                    ...
                    $.post('/register', credentials, function(response) {
                        console.log('DONE');
                        location.reload();
                    });
                });
